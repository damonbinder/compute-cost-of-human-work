#!/usr/bin/env python3
"""Reconstruct ARC1 first-test comparisons. Python 3.10+, standard library only.
...
def load_model(sources, config, tasks):
    ...
    for path in sorted((agent-work/sources/'runs'/config['source_directory']).glob('*.json')):
        ...
        first_responses = []
        incomplete = []
        for array_index, pair in enumerate(json.loads(path.read_text())):
            actual_indexes = set()
            responses = []
    ...
    return included, omitted, other_pairs, mismatches


def calculate(sources, selection, models):
    tasks = load_tasks(sources)
    humans, action_counts, untimed = load_humans(sources,tasks)
    ...
    for config in selection['configurations']:
        included, omitted, others, mismatches = load_model(sources,config,tasks)
        assert included
...
def main():
    ...
    for name in ['sources','selection','models','output']:
        p.add_argument('--'+name,type=Path,required=True)
    a = p.parse_args()
    sources, selection, models, output = [getattr(a,k).resolve() for k in ['sources','selection','models','output']]
    if output.exists() or output.is_relative_to(sources) or output in [selection,models]:
        p.error('output must be new and outside the retained source directory')
    manifest = json.loads((agent-work/sources/'source-manifest.json').read_text())
    ...
    result = calculate(sources,json.loads(selection.read_text()),model_rows)
    result['source_manifest_sha256'] = sha256(agent-work/sources/'source-manifest.json')
