...
parser.add_argument('--effort',choices=['none','low','medium','high'],required=True)
...
for file in sorted((s / args.effort).glob('*.json')):
    ...
    for array_index, row in enumerate(json.loads(file.read_text())):
        ...
        for slot, response in row.items():
            ...
            assert meta['model'] == 'gpt-5.2-2025-12-11'
            assert meta['kwargs']['reasoning']['effort'] == args.effort
            if ix != array_index:
            ...
            assert (response['answer'] == puzzle['test'][ix]['output']) == response['correct']
            usage = meta['usage']
            assert usage['total_tokens'] == usage['prompt_tokens'] + usage['completion_tokens']
            assert 0 <= usage['completion_tokens_details']['reasoning_tokens'] <= usage['completion_tokens']
    ...
    for ix, calls in pairs.items():
        ...
        out.append({
            ...
            'reasoning_tokens_in_completion': sum(a['metadata']['usage']['completion_tokens_details']['reasoning_tokens'] for _, a in calls),
            'ai_correct': any(a['correct'] for _, a in calls),
            'human_n': len(hs),
...
summary = {
    ...
    'reasoning_tokens_in_completion': sum(r['reasoning_tokens_in_completion'] * r['human_n'] for r in selected) / n,
